...
pub struct Service<S> {
    ...
    // このサーバーに紐付いているノード一覧
    members: HashMap<(BucketId, u16, entity::device::DeviceId), NodeId>,
    ...
    S: Spawn + Send + Clone + 'static,
{
            ...
            .find(|(_, device)| device.id().as_str() == device_id)
        {
            if let Some(handle) = local_device.handle() {
                for (version, segment_no) in versions {
                    if let Some(node_id) = self.get(bucket_id, segment_no, device_id) {
    ...
    pub fn inspect_physical_device(
        ...
        futures::future::result(
            ...
            handle.request().list().map(move |lump_ids| {
                info!(logger, "test: {:?}", lump_ids);
                PhysicalDeviceInspection::new(
                    device_id,
                    lump_ids.iter().map(|id| id.as_u128()).collect(),
                )
            })
        }).map_err(Error::from)
    }
    ...
    fn get(
        ...
        self.members.get(&(bucket, segment, device))
    }

    fn remove_all(&mut self, device: &entity::device::DeviceId) {
        self.members.retain(|(_, _, d), _| d != device);
    }
